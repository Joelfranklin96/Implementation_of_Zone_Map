...
struct zone
{
    ...
    unsigned int size;               // no. of elements in current zone
};
...
class zonemap
{
    ...
    T num_zones;             // keeps track of the number of zones/partitions to the array/vector of elements
    T num_elements_per_zone; // keeps track of number of elements per zone (items in every zone has to be less than or equal to this counter)
    bool sorted_flag;
...
public:
     ...
     * returns: N/A 
     */
    zonemap(std::vector<T> _elements, unsigned int _num_elements_per_zone, bool _sorted_flag);
    ...
    int binarySearch(std::vector<unsigned int> &input_data, unsigned int low, unsigned int high, unsigned int x);
};
#endif
